...
# Delimiters
comma = {',': ' symbol comma (,)'}
comma_keys = comma.keys()

# If blocks
if_block = {'if': 'Enter If Loop'}
if_block_keys = if_block.keys()

# For blocks
for_block = {'for': 'Enter For Loop'}
for_block_keys = for_block.keys()
...
def construct_grammar():
    ...
    None
    """
    global rules, nonterm_userdef, term_userdef, id1, id2, id3, id4
    rules = [
        "S -> T M B A D",
        "T -> int",
        "M -> main()",
    ...
    nonterm_userdef = ['S', 'T', 'M', 'B', 'D', 'A', 'E', 'W', 'P', 'Q', 'F', 'X', 'C']
    term_userdef = [id1, id2, id3, id4, 'expr', 'int', 'main()', 'end', 'begin', '(', ')', '+', '/', 'if', 'relop', 'for', '++', '1', '>', '=', '0', ',', ';']
...
def lexicalAnalyzer():
    ...
    Returns:None
    """
    global operators, optr_keys, comments, comment_keys, header, header_keys, macros, macros_keys, datatype, datatype_keys, keyword, keyword_keys, delimiter, delimiter_keys, while_block, while_block_keys, if_block, if_block_keys, for_block, for_block_keys,  blocks, block_keys, builtin_functions, non_identifiers, numerals, dataFlag, id1, id2, id3, id4, count
    ...
    for line in program:
        ...
        for token in tokens:
            ...
            if token in if_block_keys:
                print(if_block[token])
            if token in for_block_keys:
                print(for_block[token])
